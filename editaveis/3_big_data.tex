...
um \textit{stream} \cite{seyvet2016}. Esse fato sustenta a Arquitetura Kappa,
que se baseia no uso único da camada \textit{speed} para processamento,
descartando assim a camada \textit{batch} \cite{kreps2014}.

A Arquitetura Kappa utiliza quatro princípios principais: (i) Tudo é um
\textit{stream}; (ii) Dados imutáveis; (iii) Somente um \textit{framework};
(iv) Reprodução histórica \cite{seyvet2016}. Além disso, é importante que
a tecnologia de \textit{stream} utilizada garanta a ordem dos itens, para
que a reprodução dos mesmos dados gere o mesmo resultado \cite{seyvet2016}.
Assim, a Arquitetura Kappa funciona da seguinte forma:
