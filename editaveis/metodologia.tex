...
implementação do \textit{AppRecommender}, onde o mesmo usava comparações tanto offline
...
Para comparar as soluções desenvolvidas, os testes realizados no
\textit{AppRecommender} implementaram o uso de duas métricas distintas,
sendo uma a medida da precisão e cobertura alcançado pelos algoritmos desenvolvidos,
visando principalmente definir o tamanho do perfil do usuário ou o tamanho de vizinhança, para
...
20 submissões aleatórias do \textit{popularity-contest} para funcionar. Vale ressaltar
que cada submissão do \textit{popularity-contest} serve como forma de mostrar os pacotes
...
solução determinística quanto para a solução por aprendizado de máquina. Dessa
...
experimentalmente na solução do \textit{AppRecommender} e que as soluções implementadas
