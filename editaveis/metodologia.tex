...
As modificações foram realizadas em uma cópia local do
...
Onde cada pacote "P" possui um peso "w" com valores entre 0 e 1, esse peso
é definido pela função W(P), que foi previamente explicado, a função que
define o valor do peso de um pacote.

$W(Pi) = w_i , \quad 0 <= w_i <= 1$

Aplicando essa função para cada pacote do termo é formada a lista dos pesos
dos pacotes de cada termo, e então essa lista é ordenada, gerando a lista
dos pesos ordenados teta "$\theta$".

$sort(\{w_1, w_2, ..., w_n\}) = \{\theta_1, \theta_2, ..., \theta_n\}$

Para calcular o peso do termo é utilizado os cinco maiores pesos de pacote
associados ao termo, ou seja, se algum termo possui uma lista com tamanho "n"
menor do que cinco pacotes, onde "n" indica o tamanho da lista, para cada peso
de pacote que está faltando na lista para completar o tamanho cinco, este peso
...
Logo o valor temporal do termo é calculado pela média aritmética desses
...
\subsubsubsection{Comparação offline}
...
\subsubsubsection{Estudo com usuários}
